MODEL (
  ...
  kind CUSTOM (
    ...
    materialization_properties (
      ...
      'frozen_columns' = 'owning_account_number,placed_at,owning_account_effective_from,owning_customer_number,first_seen_late'
    )
  ),
  dialect duckdb,
  columns (
...
--
-- frozen: handoff.raw.trade_cdc.t_ca_id->logical.trade.owning_account_number and
-- handoff.raw.trade_cdc.t_dts->logical.trade.placed_at (selector first_encounter_only) from
-- the row with the smallest (batch_date, cdc_dsn) per trade_code_meanings.report_order.
-- logical.trade.first_seen_late is computed_within_entity from that same first report's
-- t_st_id against the anchored status_order (PNDG is the lifecycle's first status).
WITH first_report AS (
  SELECT
    t_id AS trade_number,
    t_dts AS placed_at,
    t_ca_id AS owning_account_number,
    t_st_id <> 'PNDG' AS first_seen_late
  FROM raw.trade_cdc
