# Change contract, cycle 11: L1 -> L2, job ownership-history (unknown codes are held, never interpreted)
...
- **Acceptance:** `.venv/bin/python scripts/chain_l2.py check ownership-history` ok with zero problems; the weave no longer lists L1.unknown-codes as a gap once all three jobs have compiled; diff confined to the enumerated elements.
