...
- **L2 selected, all three jobs:** `ownership-history` (ten cycles), `trade-lifecycle` (nine), `positions` (five). Weave: no uncovered clause, seven cross-job dependencies, twelve named gaps each pointing at an open hole.
- **L3 accepted and stamped, both engines:** `ownership-history` (16 audits each, mutation testing zero unprotected). `trade-lifecycle` and `positions` are re-projecting against the latest selections on `duckdb-native` and `duckdb-sqlmesh`; the gate rejects them until each new deterministic invariant has an audit.
- **Counterexamples:** the account-428 rollover (`counterexamples/archive/`) runs as a two-phase simulation on every job; the proposed trade-before-account-statement case (`counterexamples/proposed/`) runs with `chain_l3.py simulate` and shows the two engines disagreeing until the trade-lifecycle audits land. Proposals awaiting authority in `chain/ce/proposed/`: first-seen-late market orders, a late-arriving earlier statement, account-action uniqueness, the constructed status vocabulary, and the trade placed before its account's first statement.
...
- **Standing anchor question:** `sources-v1.json` states no status for NEW, UPDCUST or UPDACCT; "active" for those codes is a recorded parallel assumption that two engines' audits now rely on.
