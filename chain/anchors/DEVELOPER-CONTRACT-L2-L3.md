...
- **Stable projection contracts:** output under `chain/l3/<target>/<job>/`: `manifest.json` (see `chain/anchors/l3-manifest-v1.schema.json`), one SQL file per entity named `<entity>.sql`, one SQL file per deterministic invariant under `audits/<invariant id>.sql` returning zero rows when the invariant holds, and nothing else. The engine profile is `chain/profiles/<target>.json`; follow its strategies. Source tables exist under the schemas the anchors name (`raw.*`, `ce.*`); upstream entities exist as `governed.<entity name>`; you create `governed.<entity name>` for your job's entities.
