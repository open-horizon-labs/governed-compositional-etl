...
### Positions review; customer statement pin (2026-09-17)

- Positions cycle 1 reviewed: needs-authority. Customer position was summed per customer number because the trade's frozen reference carries the account statement's time and only the customer's number. L1.attribution-at-placement's "and through it the customer, as they stood" and L1.no-phantom-positions' "any one statement of an account or customer" entail the customer statement pin. Adjudicated under the domain-reviewer hat as a trade-lifecycle L2 correction (cycle 6), then positions cycle 2. Both contracts written; the L3 re-projection contracts for trade-lifecycle updated to include the new attribute. The positions reviewer also asked for change-effective-time to bound the sums, the owner-change hole to be carried, an update-in-place invariant, and an honest constructed-scenarios statement; all in the positions contract.
- Cross-level chain of one finding: an L2 review of the last job found a missing element two jobs up, which the upstream reviewer had noted as an asymmetry and accepted. The composition caught what the local review passed. Cone: sg.placement-moment stale again after cycle 6; two trade artifacts re-project; positions recompiles against the new reference.
- Counterfactual round 2 and the trade-lifecycle cycle-5 scoped review are in flight.
...
### Re-projections land; counterfactual round 2 files the question (2026-09-17)

- Trade-lifecycle re-projected on both engines from the re-selected L2: 353232 placed 2017-01-12 14:55:11 (its earliest history row) and no longer marked late; 372101 unchanged at 2017-04-10 19:28:01; both pin account 428's 2012 statement and customer 238's 2008 statement. Cross-target compare identical; two-phase keeps all six frozen attributes pinned; six audits each. Scoped projection reviews in flight for both.
- Design note recorded in the L2 to L3 contract: frozen protects against later reports, not against a changed derivation. When an L2 change alters how a frozen value is derived, existing rows must be rebuilt; the cache's stale set names the entities. The native Developer hit this on its persistent build database and cleared it.
- Counterfactual round 2 (same Sketch without the statement-content clauses, after the anchor and format repairs): gate question with zero problems; both entities content-free by design; the tempting decode recorded as a rejected step; one precise question filed. Round 1 had recovered a status from an anchor sentence. Same Sketch, different Developer behavior, because the anchors stopped licensing and the format made asking cheap. Independent review in flight. Gate quirk fixed: rejected frozen derivations can now be recorded.
- Positions cycle 2 is compiling against the re-selected trade reference.

### Sketch sufficiency: the counterfactual, concluded (2026-09-17)

- Round 1 (licensing anchors, terse question protocol): a status attribute recovered from an anchor sentence; a carry-forward re-homed after an anchor named the removed clause; no field-name facts. Mixed.
- Round 2 (anchors shape-only, questions cheap, rejected entries asked for): content-free statements, the tempting decode recorded as rejected, one precise question filed. Derivation-disciplined, with the reviewer's caveat that the format now tells the Developer what to do in this situation. The honest claim: the system of Sketch, anchors, format, and gate keeps a Sonnet Developer from recovering removed policy, and round 1 shows what happens when the anchors leak. Both rounds retained.
- Residual nudges the reviewer found were removed: "exactly one question" in the schema; entity-standing phrasing in code meanings; assumption annotations in the counterfactual copies. Left by design: the format states mechanisms conditionally and offers nullable-with-note.
- What this establishes for the phase-2 success signal: removing the clause made a fresh compile omit the content and ask, once the anchors stopped carrying policy. The role and content are derived, not asserted.

### Positions selected; ownership-history cycle 10 selected; the finding closes its loop (2026-09-17)

- Positions L2 selected at cycle 3 (46 elements, four groups fully selected). Weave: zero gaps, seven dependencies, ten named gaps. Both positions L3 Developers dispatched (duckdb-native, duckdb-sqlmesh) under `chain/l3/<target>/positions/change-contract-1.md`; this is the first job whose projection exercises L1.no-phantom-positions end to end.
- Ownership-history cycle 10 passed a scoped Opus review (`review-10.md`): five value-correctness invariants restate existing handoffs and derivations, cite subsets of their groups' clauses, recompute from anchored sources, and fire on the three mutations L3 found unprotected. The reviewer carried forward one caveat that predates the cycle: the anchors state no status for UPDCUST, UPDACCT or NEW, so "active"/"open" for those codes is a recorded parallel assumption, and any audit built on it is exactly that strong.
- On selection: L1 cache all hit (no clause moved), but the L3 gate rejected both ownership-history projections for two reasons at once: five deterministic invariants with no audit, and group fingerprints that moved because `sg.statement-content` and `sg.owner-standing` gained members. That is the L2-to-L3 seam doing what the L1-to-L2 cache cannot: a change internal to an L2 stales exactly the projections compiled from it. Cycle-3 L3 contracts written for both targets; dispatch waits for the positions runs to finish, because both jobs on one target share a build database.
- Loop closed by mechanism, not by memory: mutation testing (L3) found unprotected swaps, the finding was routed as candidate invariants to the next L2 contract, the scoped review judged them a restatement, selection staled the L3, and the L3 contract now requires audits that catch the original mutations. Four hops, each recorded in the file the next hop reads.

### Positions on both engines; the reviews find what compare cannot (2026-09-17)

- Positions cycle 1 landed on duckdb-native and duckdb-sqlmesh from two Sonnet Developers. Both found the same trap independently (Batch1 HoldingHistory rows carry no CDC columns, unlike the loader-normalized trade rows) and fixed it. Check ok, run ok, two-phase ok, mutate clean, cross-engine compare identical. End to end: after the constructed 2017 statement becomes current for account 428, holding changes and positions still key only the 2012 statement and the 2008 customer statement; both net to zero; nothing on the phantom statement.
- Both scoped Opus reviews fail, narrowly and differently. Native implemented the model's stated exclusion (a change whose ownership pin does not resolve drops out) as trade-row existence, which admits nulls into four non-nullable frozen attributes and a null aggregate key. SQLMesh has two audits that end at `< 0` and let a null sum pass. Both reviewers found the model names the unknown-trade case only as a review trigger, so no audit could exist; both found the D-only-pair existence decided in SQL where `hole.deletions` reserves it.
- The sqlmesh reviewer, reading both engines' trade projections side by side, found a divergence upstream that compare cannot see: native `trade.sql` LEFT JOINs the pins (keeps an unpinnable trade with nulls), sqlmesh inner-joins (drops it silently). Same selected L2, two behaviors, zero fixture rows. Routed as trade-lifecycle cycle 8 (two invariants that make either behavior a violation) and a constructed CE proposal, `ce.l2.trade-before-account-statement`.
- Routing: positions L2 cycle 4 (five invariants: quantity_change is the difference, positions are the sums, ownership present, current trade known, quantities present); trade-lifecycle L2 cycle 8; two L3 gate and harness extensions done now. The gate cross-checks SQLMesh governed_merge header properties against L2 mutation roles (the native MERGE guard's blind twin). The harness mutates derived attributes (swap, perturb) and non-nullable ones (null), not only frozen and per_statement. Rerun on the six projections: ownership-history and trade-lifecycle stay clean; positions shows exactly the two findings the reviewers had read by hand, quantity_change unprotected on both engines and net_quantity null unprotected on sqlmesh only. The reviewer's reading became a mechanical check within the hour.
- Ownership-history cycle-3 audits: native passed and is stamped; sqlmesh failed on one audit whose constructed arm copied the model's narrowing CASE instead of the invariant's pass-through, invisible to compare because both sides narrowed alike. Two reviewers gave opposite advice on a join filter; the coordinator chose the form that fires rather than drops and aligned both targets. Two anchor-level CE proposals filed: `(ca_id, action_ts)` uniqueness is load-bearing and undeclared; `ce.account_changes.status_id` has no declared vocabulary.
- One coordinator error caught by the chain: review-10's note told L3 Developers to join account audits through the owning customer, which would multiply rows; the L3 contracts had the right join, the Developers followed the contracts, and the native reviewer flagged the note. Corrected in place with the correction noted.

### Two L2 reviews fail on the contracts' own slips; a proposed CE becomes runnable (2026-09-17)

- Positions cycle 4 and trade-lifecycle cycle 8 both passed the gate and both failed scoped review, narrowly, and in both cases the reviewer traced the fault to the contract I wrote: two quantity invariants assigned to a group whose coverage claim disowns their subject; a completeness invariant scoped "over reports whose cdc_flag is not D", which quietly fills the deletions hole for a trade with an I row and a later D row, and a trade_history clause that demanded a row no handoff derives. The Developers followed the contracts literally and said so in prose; the reviewer counted that against the cycle, since the conflict protocol asks for one filed question instead. Contracts 5 and 9 corrected the contracts; the Developers resumed with context and landed every line; coordinator bookkeeping reviews selected cycles 4+5 and 8+9 together.
- Cache plan after selection: ownership-history all hit; trade-lifecycle stale on sg.placement-moment and sg.trade-identity; positions stale on sg.holding-attribution and sg.no-phantom-positions. The L3 gate rejects exactly the four projections of those two jobs, each naming its missing audits, and accepts both ownership-history projections unchanged. Weave: overlap 3, dependency 7, named gaps 12 (two groups now name the deletions hole), no gaps.
- The counterexample the sqlmesh positions reviewer proposed by reading two SQL files is now a document the harness runs: `counterexamples/proposed/ce-trade-before-account-statement-v1.json` adds a labeled Batch1 trade for account 428 placed six months before the account's first statement. `chain_l3.py simulate` on the current projections: silent on both engines while they disagree (native holds 900001 with three null pins, SQLMesh has no row). The test suite pins the property that this case is never both silent and agreed; after trade-lifecycle L3 cycle 4 the same command must fire exactly one audit per engine.
- Harness additions this round: counterexample documents may add raw rows (labeled at the document level, since raw tables carry no provenance column); simulate runs one document against one projection. The extended mutation harness found, on the six projections, exactly the two positions gaps the reviewers had read by hand and nothing else.

### The proposed counterexample fires on both engines (2026-09-17)

- Trade-lifecycle L3 cycle 4 landed on both engines from two fresh Sonnet Developers: two audits each, eight total, all zero on the fixture, two-phase and mutate clean. Neither Developer changed its join shape; the contract said the audits, not the join, are the contract.
- `simulate` with the constructed trade 900001 now fires exactly one audit per engine, and a different one on each: native persists the trade with three null pins and `inv.trade_ownership_pin_present` names it; SQLMesh drops the trade and `inv.every_received_trade_persisted` names it, as a blocking audit that refuses the plan. Two hours earlier the same command was silent on both while the tables disagreed. The reviewer's reading of two SQL files became an L2 change, two L3 changes, a runnable document and a test, and the divergence it found is now a violation with a name on either engine.
- Harness note: SQLMesh audits block promotion, so the runner now reads audit failures off the refused plan and reports them as violations instead of raising. The test that pins the counterexample's property tolerates a missing table count on an engine whose plan was refused.

### Positions cycle 2 on both engines; two more corners found by reading, then by running (2026-09-17)

- Positions L3 cycle 2 landed on both engines: the native predicate is now the model's rule (keep a report exactly when all four copied ownership values are non-null), the SQLMesh not-negative audits are null-sensitive, twelve audits each, mutate 14 for 14 including the perturbed derivation, cross-engine compare identical, two-phase still keeps the phantom statement empty. Both reviews fail narrowly again. Native: one false sentence in the manifest note about the other target (seeded by my contract's ambiguous parenthetical). SQLMesh: the extended updates-in-place audit resolves a report tie strictly where the native sibling tolerates it, deciding hole.batch_identity in SQL against its own comment.
- The SQLMesh reviewer designed a counterexample for that tie by reading; I made it a document and ran it before any fix: native silent (one candidate adopted, positions net zero), SQLMesh fires the audit and refuses the plan. Exactly as predicted. Two runnable proposed counterexamples now exist, each born from a reviewer reading two engines' SQL side by side, each showing a divergence compare cannot see because the fixture never reaches it.
- The native reviewer found the mirror of the earlier trade finding one level down: `inv.holding_change_current_trade_known` fires for the unresolved-pin case on SQLMesh (the upstream model publishes no row) and stays silent on native (the row exists with null pins and the predicate excludes the report). Same invariant, engine-dependent outcome. The honest resolution is text, not a new rule: the two readings coincide whenever trade-lifecycle's own two new invariants hold, so the engines differ only in a state trade-lifecycle already reports. Routed as positions cycle 6 together with the converse invariant (an eligible, fully pinned report yields exactly one row) that closes the last one-directional gap.
- The trade-lifecycle L2 took a one-sentence cycle 10 for a necessity parenthetical the native L3 review found; fingerprints unchanged, both L3 projections stayed valid through the fingerprint fallback, as designed.

### Positions cycle 3 on both engines; two contract slips, three runnable counterexamples (2026-09-17)

- Positions L3 cycle 3 landed on both engines: the converse row-existence audit (whole-pair abstention on D), the cardinality half in the sum audits, the SQLMesh tie audit made tolerant. Thirteen audits each, mutate 14 for 14, compare identical, the tie counterexample now silent on both.
- The native review failed narrowly on a sentence of mine: "align the candidate-filter phrasing" presumed `IS DISTINCT FROM 'D'` and `IS NULL OR IN ('I','U')` interchangeable. They differ exactly on a flag outside the anchored vocabulary, and in the candidate position the non-D form admits such a report, silencing the very audit the cycle added. The Developer followed the sentence; the reviewer disproved the equivalence; the fix reverted the candidate filters and kept the expected-set test. Second contract slip of the day after the NOT NULL sentence; both recorded as contract-authoring lessons: a contract may name an outcome and an authority, never assert an equivalence the Developer has to take on faith.
- A third runnable counterexample came out of that review: one holding report with cdc_flag 'X'. Before the revert the engines disagreed (SQLMesh fired the converse audit, native admitted the report and was silent); after, both fire the same audit on the same pair. The question whether the cdc_flag vocabulary is closed is now filed on both targets' manifests for anchor authority. Three proposed counterexamples, each born from a reviewer reading two engines' SQL side by side, each turned into a document the harness runs, each showing a divergence compare cannot see on the fixture.
- Positions L2 cycle 7 (four texts from review 6) is compiled and waits for the stamps before selection, so the text-only change falls through to the fingerprint fallback rather than rejecting a projection that was just accepted.

### Demonstration: two L1 changes, two downstream paths (2026-09-17)

- **Change A, a rewording.** `L1.placement-moment` reworded without change of meaning. The clause hash moved; the plan asked Jev for the one group under it; Jev returned p(behavior changes) 0.15, high confidence, keep. Cache: `sg.placement-moment` hit by Jev; every L2 group on every job a hit; no L3 artifact re-projects. Above the hash floor the system-one model, not the hash, decides.
- **Change B, a rule.** The business accepted `ce.l1.first-seen-late-market-orders`: an order sent straight to market has no pending stage and is not first seen late when its earliest report is submitted. The anchors gained `trade_type_codes` (shape only: which codes exist, which are market or limit). Jev: 0.68, medium, review. The coordinator adjudicated invalidate and recorded it in `chain/cache-adjudications.jsonl`; the plan resolves a review through that log. Cache: `sg.placement-moment` stale, all other groups hit, including every positions group (positions depends on trade ownership, not on the late flag) and every ownership-history group. L3 gate: both trade-lifecycle projections rejected, all four other projections accepted unchanged.
- **Two seams repaired on the way.** The L3 gate compared stamped fingerprints strictly, so a Jev keep would have staled a projection anyway once the review sha had moved; it now honors a recorded keep (by Jev or by adjudication) and demands adjudication on an unresolved review, with a test. And `commit` re-baselined a group under review to a hit; pending decisions now persist across commits until adjudicated or re-selected.
- **Downstream so far.** Trade-lifecycle cycle 11 compiled the amended clause: new frozen `order_type` from the anchored code, the first-seen-late derivation over the order type's first lifecycle event, the invariant restated. Review 11 failed it on one decision the clause does not make (a market order first reported PNDG, which the Developer marked late instead of leaving undefined), on "anchored" applied to a meaning the Sketch supplies, and on a gate-appeasing citation. Cycle 12 corrects those and files the question for the business. The runnable CE `ce-market-order-first-seen-submitted-v1.json` (a market buy and a limit buy on account 428, both first reported SBMT) waits for the L3 re-projection on both engines.

### The change lands on both engines, and its reviews send it back to the Sketch (2026-09-17)

- Trade-lifecycle L3 cycle 5 on both engines: order_type persisted frozen, first-seen-late derived over the order type's first lifecycle event, all audits zero, mutate clean, compare identical, and the market-order counterexample yields exactly the assertion: the market buy first seen submitted is not late, its limit twin is. Positions ran unchanged downstream on both engines.
- Both scoped reviews failed the same way: on the two cases the L2 left undefined (a market order first reported pending; an order type outside the vocabulary) each projection wrote a value, false by rank arithmetic and NULL respectively, and no audit could see it because the audit's scope excluded exactly those cases. Two Developers, two engines, one silence.
- Not fixed in SQL. The user's rule for this session: when an issue hits, the fix belongs in the Sketch and the CEs; projections are disposable. The Sketch gained `L1.unknown-codes` (a fixed vocabulary; an unknown code is held for review, never interpreted, defaulted or dropped; every job reports it) and `L1.hole.market-order-seen-pending`. CEs: `ce.l1.unknown-codes-held` accepted, `ce.l1.market-order-seen-pending` proposed, and a runnable document with a pending market order and a trade of type TXX, silent on both engines today. The earlier `ce.l3.holding-unanchored-cdc-flag` resolves under the new clause as well.
- Cache after the Sketch change: no group stales by hash, because no group cites the new clause yet; the weave reports it as an uncovered clause. That is the third downstream path: a clause nobody derives from is a gap the weave shows, not a stale set. All three jobs recompile to cover it (ownership-history 11, trade-lifecycle 13, positions 8); trade-lifecycle also carries the hole, makes first_seen_late nullable with null meaning held, and gets its own order_type-frozen invariant.

### Three more Sketch rounds from two engines' reviews (2026-09-17)

- Every L3 review of the recompiled trade-lifecycle found something the Sketch had not said, and each went back up rather than into SQL, per the rule that Sketch and CEs are what stick around: a report is held as a whole, not by field; a held report keeps its place in the order of reports but nothing else about it may be read; a trade any of whose placement-fixing facts (placement, owning account, order type) would come from a held report is held until the business answers the hole. Each landed as a clause sentence or a widened hole, each with a constructed trade added to the runnable counterexample (900004 through 900009 now: pending market order, unknown type, unknown later status, unknown flag, held first report, held first CDC report under an anchored history row).
- L2 trade-lifecycle cycles 13 to 16 compiled those; ownership-history 11 and 12 and positions 8 and 9 covered the clause with one reporting audit each. The ownership-history review caught an over-reach in the other direction: a constructed source with no anchored vocabulary is a question for authority, not a violation per row.
- Harness repairs found along the way: two-phase watched a trade row regardless of job; stamps and provenance ignored a group reached only through an audit; a stale cache decision persisted after every engine had stamped it; the L3 gate ignored a Jev keep. Each is now a rule with a test or a recorded behavior.
- Four of six projections accepted and stamped under the new clause; trade-lifecycle on both engines recompiles a last time under L2 cycle 16 (thirteen deterministic invariants, including the explicit absence audit the SQLMesh review asked for so no engine can leave a held trade's absence unchecked).

### The business answers a hole, and the cache learns to see holes (2026-09-17)

- Asked what happens when the business changes the Sketch or a CE, I probed the one case never exercised: a business *answering an open hole*. Amending a hole's text staled nothing. Changed clauses: none. Cache: all hit. Both gates: ok. A hole is policy every deferring element leans on, so answering one could have left six accepted projections untouched. Fixed: a group's fingerprint now includes the L1 holes its `gap` names, and the plan reports changed and answered holes and routes them to Jev like clauses.
- Two more defects fell out of the same probe. A group whose only member is a judgement invariant reaches no artifact and no audit, so no stamp could ever clear it and it sat stale forever; such a group is now vacuously attested. And the L3 gate compared stamped fingerprints only when the L2 review sha had moved, so an L1 clause or hole moving under an unchanged L2 left two provably stale projections reporting ok; the gate now asks both provenance questions every time. Each has a test.
- Then the real change: the business answered `L1.hole.market-order-seen-pending`. The answer is that the brokerage's own systems record a market order as pending on receipt, so that pending record is the order's own first lifecycle event and such a trade is not first seen late. `L1.placement-moment` carries it, the hole is closed, `ce.l1.market-order-seen-pending` moves to accepted.
- The blast radius, mechanically: one clause changed and one hole answered; Jev asked twice (0.59 and 0.42, low, review) and the coordinator adjudicated invalidate; exactly one group stale, `sg.placement-moment` in trade-lifecycle; ownership-history and positions all hit. The L2 gate rejected trade-lifecycle for citing a hole L1 no longer has. The L3 gate rejected exactly the two trade projections, for the stale group and for an audit naming an invariant that no longer exists. Four projections untouched.
- The answer reached the data: trade 900004 in the held-for-review counterexample, a market order first reported pending, was held with a null flag on both engines; it now persists with first_seen_late false on both, while the four trades held under the still-open `L1.hole.held-first-report-placement` keep no row. Cross-engine compare identical. One invariant retired, thirteen audits became twelve.
- Process defect caught in passing: both L3 Developers ran `stamp` themselves, which is the acceptance step. `stamp` now refuses a projection whose content changed after the review that judged it, and refuses a failed review outright. The guard is on content, so a touched-but-identical file is still the reviewed projection.

### Is the acceptance boundary actually fixed? Partly, and the rest is recorded (2026-09-17)

- Probed my own fix rather than trusting it. Editing SQL and stamping is refused. Writing your own `review.json` with a pass verdict and then stamping is not: on a shared filesystem a subagent with write access to its projection directory can do that, and no mechanism on that filesystem can stop it. What the record shows afterwards is the `reviewer` field naming who claimed the acceptance.
- Worse than the residual, the gate reported `ok` for a projection edited after acceptance. Fixed properly: `check` now answers two questions instead of one. `status` is well-formedness, which is what a Developer mid-cycle needs and which stays `ok` while they work. `acceptance` is whether this is the content a passed review accepted, and it goes false the moment the projection changes.
- The consequence that bites is upstream: `run` refuses to compile on a job whose projection is not accepted, before it loads anything. A job may be mid-cycle; what it builds on may not be. Both reasons are covered, a failed review and content that moved since acceptance.
- Honest summary of the boundary: accidental self-acceptance is now impossible, an edit after acceptance is visible at the gate and fatal upstream, and deliberate self-acceptance is a written claim with a name on it rather than a silent act. The Developer contract now states all of this, so a Sonnet Developer is told the rule as well as being held to it.
