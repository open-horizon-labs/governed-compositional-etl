...
- **L1.placement-moment** — A trade is placed at the moment of the earliest report of it held by the brokerage, as that report states its own time. A trade whose earliest held report is not the trade's first lifecycle event is treated as placed at that report and marked as first seen late, so the ownership it fixes can be reviewed. An order sent straight to market has no pending stage and is not first seen late when its earliest report is submitted. (Placement assumed under `issue-8-statement-content-assumed`; the market-order sentence accepted from `ce.l1.first-seen-late-market-orders` on 2026-09-17.)
...
- **L1.hole.held-first-report-placement** — When a trade's earliest report is held under `L1.unknown-codes` and a later report is not, is the trade placed at the earliest report the job can read, or is the trade itself held until the first report is resolved? `L1.placement-moment` speaks of the earliest report the brokerage holds; a held report is one the brokerage has but may not read.
