...
3. **Resolve dimension versions at first encounter only.** When a `trade_id` is not yet present, resolve `sk_account_id` and `sk_customer_id` from the account version current at that moment. Authority: `issue-8-incremental-key-binding-adjudication` (assumed).
...
| `dim-account.scd2-source` | How is `governed.dim_account` built from CustomerMgmt and Account CDC? | data-product owner | Named TPC-DI SCD2 rules in a DimAccount Sketch | Treating the counterexample fixture as the account dimension |
